...
def bind_api(**config):
    ...
    class APIMethod(object):
        ...
        def execute(self):
            ...
            if resp.status != 200:
                try:
                    error_msg = self.api.parser.parse_error(self, resp_msg)
                except Exception:
                    error_msg = "Twitter error response: status code = %s (%s)" % (resp.status,resp_msg)
                raise TweepError(error_msg)

            # Parse the response payload
            result = self.api.parser.parse(self, resp_msg)
