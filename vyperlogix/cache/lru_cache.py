...
import functools
import collections
from vyperlogix.misc import shelved

def lru_cache(maxsize = 255, timeout = None, callback = None, persistence = None):
    ...
    class _LRU_Cache_class(object):
        ...
        def _cache_wrapper(self, caller, *args, **kwargs):
            ...
            if (callable(self._persistence)):
                try:
                    value = self._persistence(key=key)
                    if (value):
                        return value
                except:
                    pass
            ...
            value = self._input_func(caller, *args, **kwargs) if caller != None else self._input_func(*args, **kwargs)
            if (callable(self._persistence)):
                try:
                    self._persistence(key=key,value=value)
                except:
                    pass
            cur_caller_cache_dict[key] = value
            return cur_caller_cache_dict[key]
...
if (__name__ == '__main__'):
    if (0):
        @lru_cache(maxsize=500,timeout=120)
        def fib(n):
            if n < 2:
                return n
    ...
    if (1):
        ...
        @lru_cache(maxsize=500,timeout=15,callback=cache_miss,persistence=persist)
        def fib(n):
            if n < 2:
                return n
            return fib(n-1) + fib(n-2)
